...
-- 이미 일부가 만들어진 상태에서도 안전하게 다시 실행할 수 있습니다.
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'garlic_rankings'
      and policyname = 'Anyone can read garlic rankings'
  ) then
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'garlic_rankings'
      and policyname = 'Anyone can submit garlic rankings'
  ) then
